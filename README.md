...
| 运行环境 | 版本 |
|---|---|
| Paper / Folia | 1.21.11+（api-version 固定为 1.21.11，不再兼容更早版本） |
| Velocity | 3.5.0-SNAPSHOT+（开发快照，生产建议固定到稳定版本） |
| JDK | 21+ |
...
Sparrow 系列库（不在 Maven Central）已经 shade 进了 jar 里；Redisson、jOOQ、HikariCP、Caffeine、MySQL 驱动等 Maven Central 依赖会在首次启动时由 Paper 自动下载。无需手动复制任何 `lib/` 目录。
...
**Q: 本地操作日志文件在哪？**
A: 位于 `plugins/FastSync/data/player-log/{uuid}.log`，实现是纯 Java NIO 的 append-only 顺序文件日志（每个玩家一个独立文件）。可安全删除整个 `data/player-log/` 目录来清理历史日志。
