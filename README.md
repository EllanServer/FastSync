...
- **本地操作日志**：基于 Chronicle Queue 的 append-only 顺序日志，用于审计和问题排查
...
| **顺序日志** | `net.openhft:chronicle-queue` | 每玩家独立 append-only journal（`data/player-log/{uuid}/`），本地持久化审计日志 |
...
Chronicle Queue 使用 `sun.nio` 内部 API 做 mmap，JDK 16+ 需要在启动脚本中添加以下 JVM 参数：
...
| [Chronicle Queue](https://github.com/OpenHFT/Chronicle-Queue) | 本地 append-only 玩家操作日志 |
